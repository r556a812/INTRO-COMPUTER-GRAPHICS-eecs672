...
Floor::Floor(ShaderIF* sIF, float cx, float cy, float cz, float lx, float ly, float lz, double k[3], float s, const char* texImageSource, float divide) : SceneElement(sIF, *(new PhongMaterial(k[0], k[1], k[2], 0.2, 0.2, 0.8, s, 1.0))),
...
void Floor::defineInitialGeometry()
{
  ...
  vec2 texCoords[4] =
  {
    {static_cast<float>(xyz[0])/d, static_cast<float>(xyz[4])/d},
    {static_cast<float>(xyz[1])/d, static_cast<float>(xyz[4])/d},
    {static_cast<float>(xyz[1])/d, static_cast<float>(xyz[5])/d},
    {static_cast<float>(xyz[0])/d, static_cast<float>(xyz[5])/d}
    //{0,0}, {1,0}, {1,1}, {0,1}
  };
